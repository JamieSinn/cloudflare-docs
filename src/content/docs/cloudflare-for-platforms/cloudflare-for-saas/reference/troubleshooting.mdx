...
The `Blocked` status is terminal — the custom hostname will not retry validation automatically. After the zone hold is released, send a [PATCH request](/api/resources/custom_hostnames/methods/edit/) to the custom hostname to restart the validation process. After the hostname has been validated, the zone hold can be enabled again.
